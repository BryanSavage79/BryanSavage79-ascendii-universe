...
const CATEGORIES = ["All", "Components", "Quest Items", "NFTs", "Bundles"];
 ...
 */
const bondingPrice = (basePrice, slope, supply) =>
...
function ItemCard({ item, onBuy, cartQty }) {
...
function CartPanel({ cart, items, onRemove, onConfirm, walletConnected }) {
  ...
  const totalEth = cartItems.reduce((sum, { item, qty }) => {
    ...
    // Each additional unit shifts the supply by 1, so price increases per unit
    let cost = 0;
    for (let i = 0; i < qty; i++) {
